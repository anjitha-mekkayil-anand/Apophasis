...
- [ ] **10.1** Prepare at least two real candidate texts: one that should be REFUSED (contains a disqualifying match) and one that should be NO_DISQUALIFIER_FOUND. Seed data must contain the thing being detected. *NOT SATISFIED: the corpus produced no `indeterminate`, so the unevaluated count and incompleteness marker were never exercised. Fixed by adding `compensation-floor` criterion.* — AC: *AC-3.1*, engineering rule 4.
